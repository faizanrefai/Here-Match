...
#import "OAServiceTicketL.h"
#import "OAMutableURLRequestL.h"
#import "OACallL.h"

@interface OACallL (Private)

- (void)callFinished:(OAServiceTicketL *)ticket withData:(NSData *)data;
- (void)callFailed:(OAServiceTicketL *)ticket withError:(NSError *)error;

@end

@implementation OACallL
...
- (void)callFailed:(OAServiceTicketL *)aTicket withError:(NSError *)error {
	NSLog(@"error body: %@", aTicket.body);
	self.ticket = aTicket;
	[aTicket release];
	OAProblemL *problem = [OAProblemL problemWithResponseBody:ticket.body];
	if (problem) {
...
- (void)callFinished:(OAServiceTicketL *)aTicket withData:(NSData *)data {
	self.ticket = aTicket;
	[aTicket release];
...

{
	...
	request = [[OAMutableURLRequestL alloc] initWithURL:url
											  consumer:consumer
												token:token
                                              callback:nil
...
*/
	...
	fetcher = [[OADataFetcherL alloc] init];
