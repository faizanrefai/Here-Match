...
- (void) requestURL: (NSURL *) url token: (OAToken *) token onSuccess: (SEL) success onFail: (SEL) fail {
   ...
   //  signatureProvider = [[OAHMAC_SHA1SignatureProvider alloc] init];
    // OAMutableURLRequest *request=[[OAMutableURLRequest alloc]initWithURL:url consumer:self.consumer token:token realm:@"http://herematch.com:3000/auth/twitter/callback" signatureProvider:[signatureProvider retain] nonce:nonce timestamp:timestamp];
	if (!request) return;
  ...
  //  [self _generateTimestamp];
   // [self _generateTimestamp];
    //request.nonce=nonce;
    //request.timeoutInterval=timestamp;
    
    //NSString *body = [NSString stringWithFormat:@"oauth_callback=%@",@"http://herematch.com:3000/auth/twitter/callback"];
...
//	[[NSUserDefaults standardUserDefaults] setObject:_accessToken.key forKey:@"OATK"];
//	[[NSUserDefaults standardUserDefaults] synchronize];
//	[[NSUserDefaults standardUserDefaults] setObject:_accessToken.secret forKey:@"OATS"];
//	[[NSUserDefaults standardUserDefaults] synchronize];
//	NSLog(@"SEA AT:%@,%@",_accessToken.key,_accessToken.secret);
}
...
- (NSString *) extractUsernameFromHTTPBody: (NSString *) body {
   ...
   // NSArray *keyValueArray_2 = [[tuples objectAtIndex:4] componentsSeparatedByString: @"="];
    ...
    //http://herematch.com/ws_herematch/twitter_token.php?oauth_token=sdjfdfg&oauth_token_secret=sdfgjhfgdfg&id=1
    NSString  *urlstring = [NSString stringWithFormat:@"%@twitter_token.php?id=%d&oauth_token=%@&oauth_token_secret=%@",kWebServiceUrl,[AppDelegate sharedInstance].UserId,strAccessToken,strAccessSecrect];    
